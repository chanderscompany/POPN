...
namespace PopN {
    public partial class PopNMainForm : Form {
        ...
        private PopNService.POPNEventLogWriter _eventLogWriter;
        ...
        public PopNMainForm(string[] args) {
        ...
        private void PopNMainForm_Load(object sender, EventArgs e) {
            ...
            try {
                ...
                MessageBoxEx.ShowAsync("PopN.exe.config file: " + ee.Message, 4000);
            }
        ...
        private object CreateComm(object arg) {
            ...
            if (_noService) {
                ...
                _service = new PopNService.PopNService();
                ...
                //UpdateMessageList("Initilize PopN service..");
                SendMessageToListLog("Initilize PopN service..");
                // start the service:
                InitService();
            }
            ...
            if (!isRunning) {
                ...
                for (int iTry = 0; iTry < nTries; iTry++) {
                    if (iTry == 0 || iTry == 2) {
                        ...
                        string text = "Installing PopN service.";
                       // UpdateMessageList(text);
                        SendMessageToListLog(text);
                        ...
                        // iTry == 1
                        string text = "UnInstalling PopN service...";
                        //UpdateMessageList(text);
                        SendMessageToListLog(text);
                        bool isUnInstalled = InstallService("-u");
                        if (!isUnInstalled) {
                ...
                SendMessageToListLog("PopN service is running.");
            }
        }

		private static bool InstallService(string args = "") {
            bool successful = false;
            string executableName = "PopNService.exe"; 
        ...
        private bool StartService() {
            ...
            foreach (ServiceController scTemp in scServices) {
				...
				if (scTemp.ServiceName == "POPNService") {
                    SendMessageToListLog("Checking PopN service...");
                    ...
                    if (sc.Status == ServiceControllerStatus.StopPending) {
                        ...
                        using (Process proc = new Process()) {
                            ...
                            proc.StartInfo.Arguments = "/IM PopNService.exe /F";
                            proc.StartInfo.WorkingDirectory = "";
                            proc.StartInfo.UseShellExecute = true;
                            proc.Start();
        ...
        private void SetNoServiceStatus() {
            ...
            UpdateClientText("POPN Comm Server or PopNService Lost.");
