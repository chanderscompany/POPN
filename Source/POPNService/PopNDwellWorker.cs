...
namespace PopNService {
	...
	partial class PopNDwellWorker {
            ...
            if (e.Error != null) {
                ...
                */
                ...
                if (RunningAsService) {
                    ...
                    SendStatusString(message);
                    //EventLogPopNService.WriteEntry(message, System.Diagnostics.EventLogEntryType.Information, 909);
                    if (!_noRetryOnError) {
		...
		private void ShutDown() {
            SendStatusString("##PopNService worker thread is shutting down.");
			//Communicator.UpdateStatus(new PopStatusMessage("PopNService worker thread is shutting down."));
